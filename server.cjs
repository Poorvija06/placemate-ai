...
async function generateGeminiContent(options) {
  ...
  for (let attempt = 0; attempt < availableModels.length; attempt++) {
    ...
    try {
      ...
      const isQuotaError = err?.status === "RESOURCE_EXHAUSTED" || err?.code === 429 || err?.message && (err.message.includes("429") || err.message.includes("quota") || err.message.includes("RESOURCE_EXHAUSTED"));
      lastError = err;
      if (isQuotaError) {
        modelExhaustedUntil[model] = Date.now() + 10 * 60 * 1e3;
...
function getAptitudeFallback(topic, difficulty, lang) {
  return {
...
function getDsaFallback(topic, programmingLanguage, lang) {
  ...
  return {
    ...
    codeExample: {
      language: pLang,
      code: pLang === "Python" ? `def find_max(arr):
    max_val = arr[0]
    for num in arr:
        if num > max_val:
            max_val = num
...
print("Max element:", find_max([12, 45, 2, 89, 34]))` : `public class Main {
    ...
    practiceProblem: {
      ...
      starterCode: pLang === "Python" ? `def second_largest(arr):
    ...
    public static int secondLargest(int[] arr) {
      ...
      solutionCode: pLang === "Python" ? `def second_largest(arr):
    ...
    for n in arr:
        if n > first:
            ...
            second = n
    return second` : `public class Solution {
    public static int secondLargest(int[] arr) {
...
function getProgrammingFallback(topic, programmingLanguage, lang) {
  const pLang = programmingLanguage || "Java";
  return {
...
3. Case sensitivity applies: 'count' and 'Count' are distinct variables.`,
    codeExample: {
      language: pLang,
      code: pLang === "Python" ? `student_name = "Alex"
age = 20
gpa = 8.5
is_enrolled = True

print(f"Student: {student_name}, GPA: {gpa}")` : `public class Main {
    public static void main(String[] args) {
    ...
    practiceExercise: {
      ...
      starterCode: pLang === "Python" ? `a = 5
b = 10
# Swap values here
print(f"a = {a}, b = {b}")` : `public class Main {
    public static void main(String[] args) {
        int a = 5;
        int b = 10;
        // Swap values here
...
function getCodeEvalFallback(code, programmingLanguage) {
  const pLang = programmingLanguage || "Java";
...
Output matches expected test cases.` : `Compilation Error / Empty Code`,
    feedback: hasContent ? `Good attempt! Your code structure adheres to ${pLang} syntax guidelines, compiles cleanly, and passes fundamental test conditions.` : `Please provide code logic to execute the exercise requirement.`,
...
function getSpokenEvalFallback(transcript) {
...
function getMockQuestionFallback(qnaHistory, targetCompany, domain, interviewType) {
...
function getMockEvalAnswerFallback(question, answer) {
  const ansLen = (answer || "").trim().length;
  return {
    score: ansLen > 30 ? 85 : 65,
    feedback: ansLen > 30 ? `Well structured answer! You addressed the question directly and provided good context. To improve further, quantify your achievements with numerical metrics.` : `Your answer is a bit brief. Expand with specific examples, project details, and results.`
  };
}
function getMockFinalizeFallback(qnaHistory, targetCompany) {
  return {
    score: 82,
    strengths: [
...
function getCoachChatFallback(userMessage, lang) {
...
function getCompanyPrepFallback(companyName, domain) {
...
function getDailyChallengesFallback(progLang) {
...
app.post("/api/auth/register", (req, res) => {
  try {
    ...
    db.users[cleanEmail] = {
      ...
      // In real app use bcrypt, here stored securely in local DB file
    };
  ...
  try {
    ...
    if (delta) {
      ...
      if (delta.dailyChallengeType) {
        ...
        p.dailyChallengesCompleted[todayStr][delta.dailyChallengeType] = true;
      }
    }
    ...
    p.placementReadiness = Math.round(
      p.aptitude * 0.2 + p.dsa * 0.25 + p.programming * 0.2 + p.communication * 0.15 + p.mockInterview * 0.1 + p.companyPrep * 0.1
    );
    const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    if (p.lastActiveDate) {
      const last = new Date(p.lastActiveDate).toISOString().split("T")[0];
      if (last !== today) {
        const diff = (new Date(today).getTime() - new Date(last).getTime()) / (1e3 * 3600 * 24);
        if (diff === 1) {
          p.dailyStreak += 1;
        } else if (diff > 1) {
          p.dailyStreak = 1;
    ...
    p.lastActiveDate = (/* @__PURE__ */ new Date()).toISOString();
      ...
      const aiResponse = await generateGeminiContent({
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              resumeScore: { type: import_genai.Type.INTEGER },
              atsReadiness: { type: import_genai.Type.INTEGER },
              detectedCgpa: { type: import_genai.Type.STRING },
              detectedName: { type: import_genai.Type.STRING },
              detectedEmail: { type: import_genai.Type.STRING },
              detectedPhone: { type: import_genai.Type.STRING },
              detectedEducation: {
              ...
              detectedSkills: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              detectedLanguages: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              detectedProjects: {
                type: import_genai.Type.ARRAY,
                items: {
                  type: import_genai.Type.OBJECT,
              ...
              detectedCertifications: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              detectedAchievements: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              ...
              areasForImprovement: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              missingInformation: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            conceptTitle: { type: import_genai.Type.STRING },
            simpleExplanation: { type: import_genai.Type.STRING },
            exampleProblem: {
              type: import_genai.Type.OBJECT,
              properties: {
                question: { type: import_genai.Type.STRING },
                solutionSteps: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
          ...
          properties: {
            ...
            codeExample: {
              ...
              properties: {
                ...
                lineByLineExplanation: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } }
              },
            ...
            practiceProblem: {
              ...
              properties: {
                title: { type: import_genai.Type.STRING },
                description: { type: import_genai.Type.STRING },
                starterCode: { type: import_genai.Type.STRING },
                expectedOutput: { type: import_genai.Type.STRING },
                hint: { type: import_genai.Type.STRING },
                solutionCode: { type: import_genai.Type.STRING }
              },
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            conceptTitle: { type: import_genai.Type.STRING },
            simpleExplanation: { type: import_genai.Type.STRING },
            syntaxRules: { type: import_genai.Type.STRING },
            codeExample: {
              type: import_genai.Type.OBJECT,
              properties: {
                language: { type: import_genai.Type.STRING },
            ...
            practiceExercise: {
              ...
              properties: {
                title: { type: import_genai.Type.STRING },
                description: { type: import_genai.Type.STRING },
                exerciseType: { type: import_genai.Type.STRING },
                starterCode: { type: import_genai.Type.STRING },
                expectedOutput: { type: import_genai.Type.STRING },
                hints: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
                solutionCode: { type: import_genai.Type.STRING }
              },
          ...
          properties: {
            ...
            complexity: {
              ...
              properties: {
                ...
                space: { type: import_genai.Type.STRING }
              }
            },
            bugsFound: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
            suggestions: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
            optimizedCode: { type: import_genai.Type.STRING }
          }
        }
      }
    });
    const evalResult = JSON.parse(aiResponse.text || "{}");
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            vocabularyScore: { type: import_genai.Type.INTEGER },
            clarityScore: { type: import_genai.Type.INTEGER },
            confidenceScore: { type: import_genai.Type.INTEGER },
            transcription: { type: import_genai.Type.STRING },
            grammarFeedback: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
            vocabularySuggestions: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
            improvedVersion: { type: import_genai.Type.STRING },
            generalFeedback: { type: import_genai.Type.STRING }
          }
        }
      }
    });
    const evalData = JSON.parse(aiResponse.text || "{}");
    ...
    const aiResponse = await generateGeminiContent({
      ...
      config: {
        ...
        responseSchema: {
          type: import_genai.Type.OBJECT,
          properties: {
            questionNumber: { type: import_genai.Type.INTEGER },
            question: { type: import_genai.Type.STRING },
            context: { type: import_genai.Type.STRING }
          },
          required: ["question"]
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            weaknesses: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
            suggestions: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
            areasToImprove: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } }
          }
        }
      }
    });
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            examPattern: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
            aptitudeFocus: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
            technicalFocus: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
            technicalQuestions: {
              type: import_genai.Type.ARRAY,
              items: {
                type: import_genai.Type.OBJECT,
            ...
            proTips: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } }
          }
        }
      }
    });
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              ...
              options: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              correctIndex: { type: import_genai.Type.INTEGER },
              rewardXp: { type: import_genai.Type.INTEGER }
            },
